<?php
	...
	// confirm raid roster
	if (!$error) {
		$stmt->prepare("UPDATE `raid_rosters` SET `confirmed` = TRUE WHERE `id` = ?");
	...
	// log event
	if(!$error) {
		$logDescription = "confirmed raid roster for event <a href='/event.php?id=" . $event['id'] . "'>" . $event['title'] . " (" . $event['id'] . ")</a>.";
